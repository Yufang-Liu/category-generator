...
class FullCCGTokenIndexer(TokenIndexer):
    """
    ...
    def tokens_to_indices(
        ...
        for index_name in self.related_vocabs:
            index_list = []
            for tok in tokens:
                if self.transform(tok) not in vocab.vocab[index_name]:
                    vocab.add_token_to_namespace(self.transform(tok), index_name)
                index_list.append(vocab.get_token_index(self.transform(tok), index_name))
